...
const SignupCard = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setFormTouched(true); // Mark the form as touched on submit
    validateEmail(email);
    ...
    if (!errors.email && !errors.password && email && password) {
      ...
      try {
        ...
        setEmail("");
        setPassword("");
        setFormTouched(false); // Reset form touch status after successful submission
      } catch (error) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-black">
      ...
      >
        <h1 className="text-black uppercase tracking-wider font-bold text-xl mt-6">Sign In</h1>
        ...
        {/* API Error Message */}
        {apiError && (
          <p className="text-red-500 text-xs mb-1">{apiError}</p>
        )}
